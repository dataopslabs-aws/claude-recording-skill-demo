# Daily schedule + payload delivery (Lab 4)
...
the `lab4-form-fill-forwarder` Lambda, which calls `InvokeAgentRuntime`:
...
- **Scheduler role** (`lab4-scheduler-invoke`): `lambda:InvokeFunction` on the forwarder
  only. Created by `make schedule` from `deploy/iam_scheduler_role.json`.
- **Lambda role** (`lab4-lambda-forwarder`): `bedrock-agentcore:InvokeAgentRuntime` on the
