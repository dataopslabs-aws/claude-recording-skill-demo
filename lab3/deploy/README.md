...
```bash
make dedup-table     # DynamoDB idempotency table (lab3-form-fill-dedup)
...
make deploy          # register the AgentCore runtime (lab3_form_fill)
...
make deploy-lambda   # forwarder Lambda (lab3-form-fill-forwarder) + role
...
**EventBridge Scheduler (05:00) → `lab3-form-fill-forwarder` Lambda → `InvokeAgentRuntime`
