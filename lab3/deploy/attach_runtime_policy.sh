...
#   RUNTIME=lab3_form_fill REGION=us-east-1 bash deploy/attach_runtime_policy.sh
set -euo pipefail
...
aws iam put-role-policy --role-name "$ROLE_NAME" --policy-name lab3-runtime \
  --policy-document "$POLICY"
echo "Attached lab3-runtime policy to $ROLE_NAME. Wait a few seconds, then 'make invoke'."
